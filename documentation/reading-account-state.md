...
* **NFT, cNFT, and token assets.** Assembling an asset's owner, metadata, and (for compressed NFTs) its Merkle proof from raw accounts takes custom indexing, decompression, and many calls.
...
| --- | --- |
| **Standard RPC** (standard Agave node, unless using Account Sync) | `getAccountInfo`, `getMultipleAccounts`, `getTokenLargestAccounts` |
